...
def _():
    ...
    font = {"size": 15}
    plt.rc("font", **font)
    return Cobra, Dubin4dReducedModel, Trooper, hj, interp, jnp, np, ode, plt
...
def _(d, np):
    ...
    x0 = np.array([-1, 0.0, -np.pi / 2, 0])  # initial state
    return u_fn0, x0
...
def _(
    ...
    for iter in range(10):
        ...
        Phi, _ = np.linalg.qr(cobra.Phi, mode="complete")
        Psi, _ = np.linalg.qr(cobra.Psi, mode="complete")
        Phi = Phi[:, :r]
        Psi = Psi[:, :r]
        ...
        model = Dubin4dReducedModel(Phi=trooper.Phi, Psi=trooper.Psi)
    ...
    # Get true solution
    model = Dubin4dReducedModel(Phi=np.eye(4), Psi=np.eye(4))
    ...
    xgrid = jnp.einsum("ij,...j -> ...i", model.Phi, grid.states)
    l = jnp.sqrt((xgrid[..., 0] - 1.0) ** 2 + (xgrid[..., 1] - 1.0) ** 2) - 0.1
    g = -jnp.sqrt((xgrid[..., 0] - 0.5) ** 2 + (xgrid[..., 1] - 0.0) ** 2) + 0.1
...
def _(T, np, plt, records, ts):
    ...
    fig, axs = plt.subplots(12, 3, figsize=(10, 40))
    ...
    plt.tight_layout()
    plt.savefig("/Users/dylanhirsch/Desktop/dubins_car_4d_troop_and_cobra.png")
    plt.show()
    return
